...
// 获取带有指定前缀的所有key值
func (c *Cacher) Keys(prefix string) (interface{}, error) {
	return c.Do("KEYS", prefix)
}
...
// ZRem 移除有序集 key 中的一个成员，不存在的成员将被忽略。
func (c *Cacher) ZRemRangeByScore(key string, score int64) (reply interface{}, err error) {
	return c.Do("ZREMRANGEBYSCORE", c.getKey(key), score, score)
}
...
// 执行所有事务块内的命令
func (c *Cacher) Commit() {
	c.Do("EXEC")
}

// 取消事务，放弃执行事务块内的所有命令
func (c *Cacher) Rollback() {
	c.Do("DISCARD")
}
